...
public class ErrorResponseFilter implements ClientResponseFilter {
	...
	public void filter(ClientRequestContext requestContext, ClientResponseContext responseContext) throws IOException {
		...
		if (httpStatusCode == Response.Status.OK.getStatusCode()
			...
			if (logger.isDebugEnabled()) {
				logger.debug(
						"filter(ClientRequestContext, ClientResponseContext) - Successful HTTP status returned.  Continuing... - httpStatusCode={}", //$NON-NLS-1$
						httpStatusCode);
			}
		} else {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"filter(ClientRequestContext, ClientResponseContext) - Error HTTP status returned.  Handling error condition... - httpStatusCode={}", //$NON-NLS-1$
						httpStatusCode);
			}
			...
			if (responseContext.hasEntity()) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug(
							"filter(ClientRequestContext, ClientResponseContext) - No entity found in response context.  Throwing generic exception."); //$NON-NLS-1$
				}
